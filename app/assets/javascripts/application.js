...
var RouteNavigator = function(firstStep,instructionDivTemp,distanceDivTemp, DirectionsTemp) {
  ...
  this.checkIfAtIntersection = function(coordinatesTemp) {
    ...
    for (i = 0; i < this.currentIntersectionsArray.length; i++) {
      ...
      if ( (distTemp < 8) && (this.currentIntersectionIndex != i) ) {this.currentIntersectionIndex = i; this.findNextIntersection(); this.nextIntersectionDistance = parseInt( getGeodesicDistance( this.currentIntersectionsArray[i].location,this.nextIntersection.location ) ); this.instructionDiv.innerHTML = "index : "+this.currentIntersectionIndex+" - "+distTemp+" --- "+ this.currentIntersectionsArray[i].location+ " ?? "+ this.nextIntersection.location; break;}
